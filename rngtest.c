...
int usage()
{
	fprintf(stdout,"rngtest v6.5\n");
	fprintf(stdout,"\t <-c> Exit after processing n blocks (default: 0)\n");
	fprintf(stdout,"\t <-b> Dump statistics every n blocks (default: 0)\n");
	fprintf(stdout,"\t <-t> Dump statistics every n secods (default: 0)\n");
	fprintf(stdout,"\t <-p> Enable pipe mode: work silently, and echo to stdout all good blocks\n");
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "c::b::t::p",
	...
	{
		int n;
		if (optarg && (sscanf(optarg, "%i", &n) == 0) || (n < 0))
			fprintf(stderr,"unable parse -c argv\n");
		else
	...
	case 'b':
	{
		...
		if ((sscanf(optarg, "%i", &n) == 0) || (n < 0))
			fprintf(stderr,"unable parse -b argv\n");
		else
	...
	case 't':
	{
		int n;
		if ((sscanf(optarg, "%i", &n) == 0) || (n < 0))
			fprintf(stderr,"unable parse -t argv\n");
		else
