...
static int usage()
{
	fprintf(stdout,"rngd v6.5\n");
	fprintf(stdout,"\t <-d> Enable debug output\n");
	fprintf(stdout,"\t <-f> Do not fork and become a daemon\n");
	fprintf(stdout,"\t <-i> Ignore repeated fips failures\n");
	fprintf(stdout,"\t <-b> Become a daemon (default)\n");
	fprintf(stdout,"\t <-x> Disable the numbered entropy source specified\n");
	fprintf(stdout,"\t <-n> Enable the numbered entropy source specified\n");
	fprintf(stdout,"\t <-l> List the operational entropy sources on this system and exit\n");
	fprintf(stdout,"\t <-O> rng specific options in the form source:key:value\n");
	fprintf(stdout,"\t <-o> Kernel device used for random number output (default: /dev/random)\n");
	fprintf(stdout,"\t <-r> Kernel device used for random number input (default: /dev/hwrng)\n");
	fprintf(stdout,"\t <-t> Enter test mode and report entropy production rates\n");
	fprintf(stdout,"\t <-p> File used for recording daemon PID, and multiple exclusion (default: /var/run/rngd.pid)\n");
	fprintf(stdout,"\t <-s> Number of bytes written to random-device at a time (default: 64)\n");

	fprintf(stdout,"\t <-W> Do not stop feeding entropy to random-device until at least n bits of entropy are available in the pool (default: 2048), 0 <= n <= 4096\n");
	fprintf(stdout,"\t <-q> Suppress all messages\n");
	fprintf(stdout,"\t <-v> List rngd version\n");
	fprintf(stdout,"\t <-e> Number of entropy bits to support (default: 8), 1 <= n <= 8\n");
}
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "dfibxnlO:o:r::tp::s::W::qve::",
	...
	case 'o':
		strcpy(arguments->random_name,optarg);
		break;

	case 'O':
		search = strchrnul(optarg, ':');
		...
		*search = ':';
		...
		while (options && options->key) {
			if (!strcmp(optkey, options->key)) {
				options->int_val = val;
				message(LOG_CONS|LOG_INFO, "Option %s found for source idx %d\n", optkey, idx);
				break;
			}
			options++;
		}
	...
	case 'l':
		...
		break;
	case 'p':
		strcpy(arguments->pid_file,optarg);
		break;
	case 'r':
		entropy_sources[ENT_HWRNG].rng_fname = optarg;
		break;
	case 'f':
	...
	case 'e': {
		int e;
		if ((sscanf(optarg,"%i", &e) == 0) || (e < 0) || (e > 8))
		{
			usage();
			exit(EXIT_FAILURE);
		}
